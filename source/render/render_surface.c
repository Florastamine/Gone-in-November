 ...
 * 
 * Turns an object into a single light ray.
 * This does not required any special hardware features, 
 * but one have to be careful with placing and scaling these 
 * "fixed" light rays in the level in order for the final rendered
 * scene to look good.
 */
void smtl_lrays_add(ENTITY *object)
{
	if(object) {
		my->flags |= (PASSABLE | TRANSLUCENT | UNLIT | BRIGHT);
		my->scale_z = 25.0;
		...
		while(my)
		{
			vec_set( &my->pan, vector(camera->pan + 180.0, 0, 0) );
